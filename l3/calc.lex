...
{endline}   {return ENDLINE;}
        /*{comment}   {yylval.com = new std::string(yytext); return COMMENT;}*/
{num}       {yylval.val = atoi(yytext); return NUM;}
"+"     { yylval.op = yytext[0]; return PLUS;}
"-"     { yylval.op = yytext[0]; /*new std::string (yytext);*/ return MINUS;}
"*"     { yylval.op =yytext[0]; /*new std::string (yytext);*/ return MULT;}
"/"     { yylval.op =yytext[0]; /*new std::string (yytext);*/ return DIV;}
"%"     { yylval.op =yytext[0]; /*new std::string (yytext);*/ return MOD;}
"^"     { yylval.op =yytext[0]; /*new std::string (yytext);*/ return POW;}
"("     { yylval.op =yytext[0]; /*new std::string (yytext);*/ return LEFT;}
")"     { yylval.op =yytext[0]; /*new std::string (yytext);*/ return RIGHT;} 
